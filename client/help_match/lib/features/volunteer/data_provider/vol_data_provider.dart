...
class VolunteerDataProvider {
  ...
  final Dio _dio;
  final LocationLocalProvider _locationLocalProvider;

  VolunteerDataProvider(
      {required dio, required sec, required locationLocalProvider})
      : _secureStorage = sec,
        _dio = dio,
        _locationLocalProvider = locationLocalProvider;
  Future<dynamic> fetchOrgs(SearchDto dto, String queryParams) async {
    queryParams += "&page_size=4";
    try {
      LatLng loc = await LocationProvider.getCurrentLocation(_locationLocalProvider);
